...
// ADR-020). Restyled with this app's own tokens (text-flow-*, bg-primary,
// bg-card) rather than shadcn's generic defaults, to match everything else
// already built this session. Onboarding's date picker is the one consumer
...
function Calendar({ className, classNames, showOutsideDays = true, ...props }: DayPickerProps) {
  return (
    <DayPicker
      ...
      classNames={{
        months: 'flex flex-col gap-flow-4',
        month: 'flex flex-col gap-flow-3',
        month_caption: 'flex items-center justify-center px-flow-8 text-flow-nav font-medium text-foreground',
        ...
        weekdays: 'flex',
        weekday: 'flex-1 pb-flow-2 text-center text-flow-micro text-muted-foreground/60',
        week: 'flex w-full',
        day: 'flex-1 p-0.5',
        day_button: cn(
          buttonVariants({ variant: 'ghost' }),
          'size-10 w-full rounded-full p-0 text-flow-small font-normal text-foreground/80',
        ),
