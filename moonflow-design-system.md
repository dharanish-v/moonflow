...
- **Vertical placement follows composition, not a blanket rule** (ADR-026): a screen that is one centered composition — icon/illustration + title + a line or two, nothing to scan — vertically centers within the available screen height (`.screen--centered`): Home (the "five-second glance"), onboarding, PIN lock, and Insights' own "not enough history yet" empty state. A screen that is a scannable list — Calendar's grid, Settings' rows, Insights once real data exists — stays top-anchored, because scanning starts at the top and centering a list just moves its starting point around unpredictably as content length varies. The log-entry sheet is neither — a partial-height sheet, not a full screen — and never stretches to fill available space regardless.
